<!-- GUIDELINES REDIRECT STUB -->

# תוכן הועבר למסמך הראשי

כל ההנחיות אוחדו ב-`COPILOT_GUIDELINES.md`.

קובץ זה נשמר כ-stub בלבד למניעת שבירת קישורים היסטוריים.

Last updated (stub): 2025-09-01
...
פירוט מלא נמצא במסמך הארכיטקטורה המרכזי. להלן רק נקודות קריטיות לשימוש יומי:

- Zustand כ-State גלובלי: userStore / workoutStore / historyStore / themeStore.
- Supabase כמקור יחיד: אין קריאות לשרת מקומי.
- Unified Questionnaire: גרסה חדשה + תמיכה לאחור דרך שכבת התאימות.
- WGER Integration: שירות מאוחד עם מטמון חכם (v2.0.0).

## 🛠️ דפוסי עבודה קריטיים (השלמות)

רוב הכללים ב-COPILOT_GUIDELINES. תוספות:

- SafeAreaView: חובה במסכים חדשים + בעת דיווח חפיפה.
- Imports: לאחר מחיקה – grep לשאריות.
- צבעים/Spacing: רק מ-`theme` (אין hex ישיר).
...
כלי עיקרי: "Identify → Extract → Replace → Remove legacy".
שלבי הסרה מדורגת: UI → Logic → Styles → Docs.
...
- אל תמחק פונקצונליות עיקרית – רק דמו/placeholder.
...
## 🧪 בדיקות איכות (מינימום הכרחי)

1. `npx tsc --noEmit`
2. הרצת בדיקות רלוונטיות (אם נוספה לוגיקה חדשה)
3. עין על לוגים: החזרות חריגות / אזהרות
...
## 🎯 יצירת משתמשים אמיתיים - עקרונות

מטאדטה לבדה לא מספיקה: היסטוריה ותוכניות חייבות להכיל תוכן מלא (תרגילים+סטים). ודא התאמת מגדר וסנכרון stores.

## 🧹 ניקוי (Workflow)

1. מחיקה
2. Grep לשאריות imports
3. עדכון Docs
4. `npx tsc --noEmit`

---

מסמך זה הוא שכבת השלמה ל-`COPILOT_GUIDELINES.md` ולא חוזר על תוכן זהה. בעת הוספת סעיף חדש בדוק קודם שאינו כבר קיים בקובץ הראשי.
