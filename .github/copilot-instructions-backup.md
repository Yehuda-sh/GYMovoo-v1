# ✨ עדכון הנחיות עבודה (2025-08-11)

- תמיד לעבוד על בסיס קוד קיים, לשמור עיצוב וצבעים, לא למחוק קבצים בלי אישור.
- להתחיל בתשתיות (שרת מקומי, בדיקות אוטומטיות), אחר כך לעבוד לפי הפלואו – כל מסך יסומן כ"סיים" רק לאחר שכל הלוגיקה, UI, בדיקות ותיעוד הושלמו.
- שרת מקומי, מוכן למעבר לשרת חיצוני.
...
## 🌐 מקור אמת: שרת בלבד (2025-08-13)

- כל הנתונים מגיעים מהשרת בלבד: משתמשים, שאלון, היסטוריית אימונים, סטטיסטיקות.
- אין דמו/placeholder/seed בצד לקוח. AsyncStorage משמש כ-cache בלבד ולא כמקור אמת.
- שימוש מחייב ב-API קיימים: `src/services/api/userApi.ts`, `src/services/api/workoutApi.ts`.
- קונפיגורציית שרת: `EXPO_PUBLIC_STORAGE_BASE_URL` חובה. באנדרואיד אמולטור – `http://10.0.2.2:3001`.
- Zustand (userStore וכו'): נטען מהשרת ומסנכרן חזרה לשרת מידי או בדיבאונס.
- אם השרת לא זמין – אין התקדמות לפיצ'רים קריטיים (למשל מעבר ל-Home ללא שאלון שלם); הצג שגיאה ידידותית.
- אין ליצור נתונים “מהאוויר” או לשנות מבנים ללא ערכים חוקיים שמוגדרים במערכת.
...
- מקור אמת: השרת בלבד. אין דמו/placeholder מצד הלקוח; AsyncStorage כ-cache בלבד.
...
- הרץ רק פקודות נחוצות עם הסבר קצר למטרה ולתוצאה המצופה; הימנע מפקודות "סרק"/"דופק" (למשל: `node -e "console.log('...')"`).
