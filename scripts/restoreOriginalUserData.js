...
function createMinimalQuestionnaire(originalSmartData) {
  ...
  if (answers.experience) minimalQuestionnaire.experience_level = answers.experience;
  if (answers.frequency) minimalQuestionnaire.workout_frequency_target = answers.frequency;
  if (answers.duration) minimalQuestionnaire.workout_duration_preference = answers.duration;
  if (answers.location) minimalQuestionnaire.workout_location = answers.location;
  if (answers.time_preference) minimalQuestionnaire.workout_time_preference = answers.time_preference;
...
function createMinimalPreferences(originalSmartData, questionnaire) {
  ...
  if (questionnaire.specific_goals) preferences.fitness_goals = questionnaire.specific_goals;
  if (questionnaire.experience_level) preferences.experience_level = questionnaire.experience_level;
  if (questionnaire.workout_frequency_target) preferences.workout_frequency = questionnaire.workout_frequency_target;
  if (questionnaire.workout_duration_preference) preferences.workout_duration = questionnaire.workout_duration_preference;
  if (questionnaire.workout_time_preference) preferences.workout_time = questionnaire.workout_time_preference;
  if (questionnaire.available_equipment) preferences.preferred_equipment = questionnaire.available_equipment;
...
async function restoreOriginalUserData() {
  ...
  for (const user of demoUsers) {
    ...
    try {
      ...
      const minimalQuestionnaire = createMinimalQuestionnaire(userData.smartquestionnairedata);
      const minimalPreferences = createMinimalPreferences(userData.smartquestionnairedata, minimalQuestionnaire);

      console.log(`📋 שאלון מינימלי עבור ${user.name}:`);
      console.log("   נתונים שיישמרו:", Object.keys(minimalQuestionnaire).join(", "));
      ...
      const removedKeys = currentKeys.filter(key => !minimalKeys.includes(key));
      ...
      console.log(`   📊 גיל: smart=${smartData.age || 'לא מוגדר'}, current=${currentQuestionnaire.age || 'לא מוגדר'}, minimal=${minimalQuestionnaire.age || 'לא מוגדר'}`);
      console.log(`   ⚡ רמת פעילות: smart=${smartData.activity_level || 'לא מוגדר'}, current=${currentQuestionnaire.activity_level || 'לא מוגדר'}, minimal=לא נכלל`);
      console.log(`   😴 שעות שינה: smart=${smartData.sleep_hours || 'לא מוגדר'}, current=${currentQuestionnaire.sleep_hours || 'לא מוגדר'}, minimal=לא נכלל`);
      console.log(`   💪 מוטיבציה: smart=${JSON.stringify(smartData.motivation || [])}, current=${JSON.stringify(currentQuestionnaire.motivation_factors || [])}, minimal=לא נכלל`);
  ...
  console.log("⚠️  זה יסיר נתונים שיצרתי בסקריפט ויותיר רק את הנתונים המקוריים");
