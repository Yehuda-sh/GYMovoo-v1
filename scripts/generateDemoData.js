...
function generateRealisticWorkout(userId, workoutIndex, daysBack) {
  ...
  workoutDate.setDate(workoutDate.getDate() - Math.floor(Math.random() * daysBack));
  
  const workoutTemplates = [
    {
      name: 'אימון כוח עליון',
      exercises: [
        { name: 'לחיצת חזה', equipment: 'dumbbells', sets: 3, targetReps: 10, targetWeight: 15 },
        { name: 'משיכת גב', equipment: 'dumbbells', sets: 3, targetReps: 12, targetWeight: 12 },
        { name: 'לחיצת כתפיים', equipment: 'dumbbells', sets: 3, targetReps: 8, targetWeight: 8 },
        { name: 'כיפופי זרועות', equipment: 'dumbbells', sets: 3, targetReps: 12, targetWeight: 10 }
      ]
    },
    {
      name: 'אימון רגליים',
      exercises: [
        { name: 'סקוואט', equipment: 'bodyweight', sets: 4, targetReps: 15, targetWeight: 0 },
        { name: 'לונג\'ים', equipment: 'dumbbells', sets: 3, targetReps: 12, targetWeight: 8 },
        { name: 'העלאת חזה', equipment: 'bodyweight', sets: 3, targetReps: 12, targetWeight: 0 },
        { name: 'הרמת שוקיים', equipment: 'dumbbells', sets: 3, targetReps: 15, targetWeight: 12 }
      ]
    },
    {
      name: 'אימון גוף מלא',
      exercises: [
        { name: 'בורפי', equipment: 'bodyweight', sets: 3, targetReps: 8, targetWeight: 0 },
        { name: 'שכיבות סמיכה', equipment: 'bodyweight', sets: 3, targetReps: 10, targetWeight: 0 },
        { name: 'פלאנק', equipment: 'bodyweight', sets: 3, targetReps: 30, targetWeight: 0 },
        { name: 'קפיצות כוכב', equipment: 'bodyweight', sets: 3, targetReps: 20, targetWeight: 0 }
      ]
    }
  ];
  ...
  const exercises = template.exercises.map((ex, index) => {
    ...
    return {
      ...
      name: ex.name,
      category: ex.name.includes('רגל') ? 'legs' : ex.name.includes('חזה') ? 'chest' : 'general',
  ...
  const workoutData = {
    ...
    completedSets: exercises.reduce((total, ex) => total + ex.sets.filter(s => s.completed).length, 0)
  };
...
function generateGamificationState(workouts, userType) {
  ...
  return {
    ...
    last_workout_date: workouts[0]?.workout.startTime || new Date().toISOString()
  };
}
...
function generateTrainingStats(workouts, userType) {
  ...
  const avgDuration = workouts.reduce((sum, w) => sum + w.stats.duration, 0) / workouts.length / 60;
  const avgVolume = workouts.reduce((sum, w) => sum + w.stats.totalVolume, 0) / workouts.length;
  ...
  return {
    ...
    averageRating: workouts.reduce((sum, w) => sum + w.feedback.enjoyment, 0) / workouts.length,
    lastWorkoutDate: workouts[0]?.workout.startTime,
    progressionRate: userType === 'premium' ? 'adaptive' : 'linear',
    strengthGains: userType === 'premium' ? {
      'לחיצת חזה': 15.5,
      'סקוואט': 22.3,
      'משיכת גב': 12.8
    } : {}
...
async function updateUserWithDemoData(userConfig) {
  ...
  try {
    ...
    for (let i = 0; i < userConfig.workoutCount; i++) {
      const workout = generateRealisticWorkout(userConfig.id, i, userConfig.daysBack);
      workouts.push(workout);
    }

    // סידור כרונולוגי (הכי חדש ראשון)
    workouts.sort((a, b) => new Date(b.workout.startTime).getTime() - new Date(a.workout.startTime).getTime());

    // יצירת נתוני גיימיפיקציה
    const gamificationState = generateGamificationState(workouts, userConfig.subscriptionType);
    
    // יצירת הישגים
    const achievements = generateAchievements(workouts, gamificationState);
    
    // יצירת training stats
    const trainingStats = generateTrainingStats(workouts, userConfig.subscriptionType);

    // עדכון subscription status
    const subscription = userConfig.subscriptionType !== 'free' ? {
    ...
    console.log(`✅ ${userConfig.name}: ${workouts.length} אימונים, ${achievements.length} הישגים, רמה ${gamificationState.level}`);
    
    // סיכום פרטים
    console.log(`   📊 נפח כולל: ${trainingStats.totalVolume}ק"ג`);
    console.log(`   ⏱️ זמן כולל: ${Math.floor(trainingStats.totalDuration / 3600)}:${Math.floor((trainingStats.totalDuration % 3600) / 60)} שעות`);
