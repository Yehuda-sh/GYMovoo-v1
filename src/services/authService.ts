...
export const fakeGoogleSignIn = async (): Promise<User> => {
  ...
  // 🎯 שאלון מדומה כדי לחסוך זמן המשתמש!
  // Mock questionnaire data to save user time
  ...
  // Return user WITH questionnaire to skip onboarding flow and save time
  const result = {
    ...randomUser,
  ...
  console.warn(
    "✅ fakeGoogleSignIn - Created user with questionnaire to skip onboarding:",
    {
      ...
      questionnaireLength: result.questionnaire?.length || 0,
    }
  );
