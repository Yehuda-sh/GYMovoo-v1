...
/**
 * יצירת נתונים אישיים ריאליסטיים לדמו/סימולציה
 * Used by: workoutSimulationService
 */
export const createRealisticPersonalData = (
  gender: "male" | "female" | "other",
  experience: "beginner" | "intermediate" | "advanced"
): PersonalData => {
  // Convert gender to PersonalData format
  const personalGender: "male" | "female" =
    gender === "other" ? "male" : gender;

  // Generate realistic age based on experience
  const ageRanges = {
    beginner: ["18_24", "25_34"],
    intermediate: ["25_34", "35_44", "45_54"],
    advanced: ["35_44", "45_54", "55_64"],
  };
  const age =
    ageRanges[experience][
      Math.floor(Math.random() * ageRanges[experience].length)
    ];

  // Generate weight ranges (different for gender)
  const weightRanges = {
    male: ["70_79", "80_89", "90_99"],
    female: ["50_59", "60_69", "70_79"],
  };
  const weight =
    weightRanges[personalGender][
      Math.floor(Math.random() * weightRanges[personalGender].length)
    ];

  // Generate height ranges (different for gender)
  const heightRanges = {
    male: ["170_179", "180_189", "190_199"],
    female: ["160_169", "170_179", "180_189"],
  };
  const height =
    heightRanges[personalGender][
      Math.floor(Math.random() * heightRanges[personalGender].length)
    ];
...
/**
 * הדפסת סיכום נתונים אישיים (לדיבוג)
 */
export const logPersonalDataSummary = (
  personalData: PersonalData,
  ...
  console.warn(`${prefix}:`, {
    ...
    weight: extractMidValueFromRange(personalData.weight) + "kg",
    height: extractMidValueFromRange(personalData.height) + "cm",
    fitnessLevel: personalData.fitnessLevel,
    bmi: calculatePersonalizedBMI(personalData),
  });
};
